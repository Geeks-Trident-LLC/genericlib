...
class TestTabularTextPatternByVarColumns:
    ...
    @pytest.mark.parametrize(
        "text,columns_count,headers_data,expected_template_snippet,expected_template,expected_result",
        [
            (
                dedent("""
                    LastWriteTime          Name
                ...
                dedent("""
                    LastWriteTime          Name
                    start() mixed_word(var_lastwritetime, at_most_2_group_occurrences)  mixed_word(var_name) end() -> record
                """).strip(),
                dedent(r"""
                    ################################################################################
                    # Template is generated by template Pro Edition
                    # Created date: YYYY-mm-dd
                    ################################################################################
                    Value lastwritetime ([\x21-\x7e]+( +[\x21-\x7e]+){,2})
                    Value name ([\x21-\x7e]+)
            ...
            (
                ...
                dedent(r"""
                    ...
                    Value drinks ([a-zA-Z0-9]+( +[a-zA-Z0-9]+){,1})
    ...
    def test_to_template_snippet(
        self, text, columns_count, headers_data,
        expected_template_snippet, expected_template,
        expected_result
    ):
        node = TabularTextPatternByVarColumns(
            text, columns_count=columns_count, divider='  ', headers_data=headers_data
        )
        tmpl_snippet = node.to_template_snippet()
        assert tmpl_snippet == expected_template_snippet
