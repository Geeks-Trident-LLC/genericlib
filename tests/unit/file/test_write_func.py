...
This test suite verifies the behavior of the `write` function, which
opens a file in text mode using `get_file_stream`, writes the provided
string content, and overwrites any existing data. The tests ensure
correct handling of file creation, overwriting, encoding, and error
conditions.
...
- test_write_with_different_encoding
...
- test_write_permission_denied_triggers_os_error
    Confirms that attempting to write to a read‑only file raises an OSError.

Usage
-----
Run pytest in the project root to execute these tests:
    $ pytest /work_dir/tests/unit/file/test_write_func.py
    or
    $ python -m pytest /work_dir/tests/unit/file/test_write_func.py
"""
